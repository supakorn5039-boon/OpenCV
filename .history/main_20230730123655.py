...
def createOverlay(width, height, count):
    overlay = np.ones((height, width, 3), dtype=np.uint8) * 255  # Create a white background overlay
    cv2.putText(overlay, str(count), (10, height - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 0), 2, cv2.LINE_AA)
    return overlay
