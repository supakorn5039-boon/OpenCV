...
def drawCount(imgPro, count, x, y):
    ...
    # Resize the img_text to match the background rectangle's size
    img_text_resized = cv2.resize(img_text, (text_width + 10, background_height))

    # Crop or resize the arrays to match the background size
    if img_bg.shape[0] != img_text_resized.shape[0]:
        if img_bg.shape[0] < img_text_resized.shape[0]:
            img_text_resized = img_text_resized[:img_bg.shape[0], :img_bg.shape[1]]
        else:
            img_bg = img_bg[:img_text_resized.shape[0], :img_text_resized.shape[1]]

    # Make sure both arrays have the same number of channels
    img_text_resized = img_text_resized[:,:,:3]

    # Combine the original image and the text overlay using the mask
    imgPro[y:y + background_height, x:x + text_width + 10] = cv2.addWeighted(img_bg, 1, img_text_resized, 1, 0)
