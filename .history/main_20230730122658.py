...
def createOverlay(width, height, count):
    ...
    overlay[:] = (255, 0, 255)  # Set the overlay color to purple (BGR value)
    cv2.putText(overlay, str(count), (5, height - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1, cv2.LINE_AA)
    return overlay
...
def checkParkingSpace(imgPro):
    for pos in posList:
        ...
        if imgCrop.shape[0] > 0 and imgCrop.shape[1] > 0:
            ...
            overlay = createOverlay(width=50, height=20, count=count)
            rows, cols, _ = overlay.shape
            roi = img_copy[y:y + rows, x:x + cols]

            # Blend the overlay with the ROI using cv2.addWeighted()
            img_copy[y:y + rows, x:x + cols] = cv2.addWeighted(roi, 0.6, overlay, 0.4, 0)
