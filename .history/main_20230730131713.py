...
def drawCount(imgPro, count, x, y):
    ...
    # Ensure the overlay arrays have the same size as the region of interest (ROI)
    img_text_resized = img_text[:background_height, :text_width + 10]
    img_bg_resized = img_bg[:background_height, :text_width + 10]

    # Overlay the text on the background image in the region of interest (ROI)
    imgPro[y + height - background_height:y + height, x:x + text_width + 10] = cv2.addWeighted(img_bg_resized, 1, img_text_resized, 1, 0)
