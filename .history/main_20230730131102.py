...
def drawCount(imgPro, count, x, y):
    ...
    img_bg = img_bg[:background_height, :text_width + 10]
    img_text_resized = img_text_resized[:background_height, :text_width + 10]

    # Combine the original image and the text overlay using the mask
    imgPro[y:y + background_height, x:x + text_width + 10] = cv2.add(img_bg, img_text_resized)
