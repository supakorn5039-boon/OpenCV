...
def drawCount(imgPro, count, x, y):
    # Create a background rectangle with reduced alpha to make it transparent
    overlay = imgPro.copy()
    cv2.rectangle(overlay, (x, y + height - 20), (x + 50, y + height), (255, 255, 255), -1)
    cv2.addWeighted(overlay, 0.5, imgPro, 0.5, 0, imgPro)

    # Draw the count number as white text
    cv2.putText(imgPro, str(count), (x + 5, y + height - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1, cv2.LINE_AA)
